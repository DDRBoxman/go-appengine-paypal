...
func (pClient *PayPalClient) SetExpressCheckoutDigitalGoods(paymentAmount float64, currencyCode string, returnURL, cancelURL string, goods []PayPalDigitalGood) (*PayPalResponse, error) {
	return pClient.SetExpressCheckoutDigitalGoodsForSeller(paymentAmount, currencyCode, returnURL, cancelURL, goods, "")
}

func (pClient *PayPalClient) SetExpressCheckoutDigitalGoodsForSeller(paymentAmount float64, currencyCode string, returnURL, cancelURL string, goods []PayPalDigitalGood, seller string) (*PayPalResponse, error) {
	values := url.Values{}
	...
	if len(seller) != 0 {
		values.Add("PAYMENTREQUEST_0_SELLERPAYPALACCOUNTID", seller)
	}
